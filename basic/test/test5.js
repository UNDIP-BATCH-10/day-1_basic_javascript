function muridTerbaik(nilai) {
  ...
  for (var key in nilai) {
    ...
    var average =
      nilai[key].reduce(function (a, b) {
        return a + b;
      }) / nilai[key].length;
    nilaiMurid.push([key, average]);
  }
  // filter undefined from nilaiMurid
  nilaiMurid = nilaiMurid.filter(function (n) {
    return n != undefined;
  });

  // get the highest average
  var highestAverage = nilaiMurid.reduce(function (a, b) {
    return a[1] > b[1] ? a : b;
  })[1];

  // get the highest average student
  var highestAverageStudent = nilaiMurid.filter(function (n) {
    return n[1] === highestAverage;
  });
